...
Fresh `izaro` install notes, including direct SSD and external SSD bootstrap paths: [docs/izaro-install.md](docs/izaro-install.md).

Fresh `izaro` install with the disk-selection wizard:
...
Short wrapper for the same `izaro` install flow from a NixOS installer or another Linux system with Nix:
...
- `izaro`: current Windows gaming and coding workstation target.
...
`dominus` and `izaro` are the current concrete `nixosConfigurations`. `exile`, `wsl-home`, and `wsl-work` remain planned targets. Add or change host outputs only after collecting the machine's hostname, architecture, users, boot mode, disk strategy, hardware config, secrets policy, and activation workflow.
...
- [Izaro install](docs/izaro-install.md)
